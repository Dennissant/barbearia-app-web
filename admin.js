...
function carregarAgenda() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            ...
            const prods = d.produtos && d.produtos.length ? `<br><small style="color:#d4af37">🛒 ${d.produtos.map(p=>p.nome).join(', ')}</small>` : '';
            const dataShow = modoAtual === 'mes' ? `<span style='color:#d4af37'>${d.data.split('-')[2]}/${d.data.split('-')[1]}</span> - ` : '';
            
            li.innerHTML = `<div style="width:100%"><div style="display:flex;justify-content:space-between"><span>${dataShow}${d.hora}</span><span style="border:1px solid #555;padding:2px 5px;border-radius:4px">R$ ${totalCli.toFixed(2)}</span></div><strong>${d.nome}</strong><br><small>${d.servico}</small>${prods}<br><a href="https://wa.me/55${d.telefone.replace(/\D/g,'')}" target="_blank" style="color:#25D366;font-size:0.8rem">📞 WhatsApp</a></div><button onclick="deletar('${doc.id}')" style="margin-left:10px;color:red;background:none;border:1px solid red;border-radius:5px">X</button>`;
            listaAgendamentos.appendChild(li);
        });
        valorTotalDia.innerHTML = total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    }, (error) => {
        if (error.message.includes("requires an index")) {
            console.log("CLIQUE AQUI:", error.message);
